...
!!! info "We're overhauling our analytics integration"
    ...
    The analytics integration is inherited from Material for MkDocs and is
...
Zensical integrates natively with Google Analytics 4. If you already set up
Google Analytics and have a property, enable it by adding the following lines to
your configuration:
...
Both properties, `title` and `ratings`, are required. Note that it's possible to
define more than two ratings, e.g. to implement a 1-5 star rating.
You can add arbitrary HTML tags to the note which is shown after the user
submitted the feedback, e.g., to link to a feedback form.

Since the feedback widget sends data to a third-party service, it is, of course,
...
used to configure the custom integration:
